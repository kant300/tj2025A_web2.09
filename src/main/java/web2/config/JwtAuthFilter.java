...
public class JwtAuthFilter extends OncePerRequestFilter {
    // 내가 만든 토큰(jwtService) 인증 방법을 (시큐리티방식)usernamepasswordauthentionToken 통합

    // [1] 내가 만든 토큰 방식
    private final JwtService jwtService;

    // [2]  기존 시큐리티 방식의 필터 커스텀, 상속받기( OncePerRequestFilter ), 물려받기 doFilterInternal 재정의
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if( request.getCookies() != null ){ // 쿠키가 존재하면
            for(Cookie cookie : request.getCookies() ){ // 모든 쿠키들을 반복문 돌려서
                if( cookie.getName().equals("loginUser")){ // 로그인 제 쿠키가 존재하면
...
//                    List.of(new SimpleGrantedAuthority("ROLE_권한명1"), new SimpleGrantedAuthority("ROLE_권한명1") ) );
            ...
            // 2-3 : 시큐리티가 사용할수 잇게 토큰 저장 . SecurityContext( 시큐리티메모리 )

...
//    }
    ...
    public ResponseEntity<?> login(@RequestBody UserDto userDto, HttpServletResponse response ) {
        UserDto result = userService.login( userDto);
        if (result != null) { // 만약에 로그인 성공했다면 성공한 유저의 아이디를 *쿠키*에 저장
            // *로그인 정보를 세션에 저장하면 서버으므로 안전하다. 쿠키(클라이언트) 저장하면 위험하다.
            ...
            // 클라이언트에서 해당 쿠키를 노출(탈취) 방지 = 주로 민감한 정보는 httpOnly 설정한다.
            cookie.setHttpOnly( true ); // .setHttpOnly( true ) : 무조건 http 에서만 사용, 즉] JS로 접근 불가능
            cookie.setSecure( false  );  // http 이용하여 탈취하더라도 암호화, 단 https(true) 에서만 가능하므로 테스트단계에서는 false 한다.
            // + 설정
...
// 세션 : 서버에 저장하는 임시 저장소이므로 서버가 종료되면 사라진다.
// 쿠키 : 클라이언트에 저장하는 임시 저장소이므로 서버거 종료되어도 유지된다.

...
public class ExamService {
    ...
    public ExamEntity put (ExamEntity examEntity){
        ...
        // 2. save( 수정할 엔티티 ) : 만약에 지정한 엔티티에 pk가 없으면 * 생성 , 만약에 지정한 엔티티에 pk가 존재하면 * 수정
        ExamEntity updateEntity = examRepository.save( examEntity );
        return updateEntity;
    }
    // 3-2. U , 특정한 엔티티 수정 , 주의할점 : 엔티티를 setter 하면 자동으로 DB도 변경된다.
    @Transactional // 트랜잭션이란 ? 여러개 sql에서 하나라도 실패이면 모두 실패
    public ExamEntity put2(ExamEntity examEntity ){
        ...
        // 2. Optional이란 ? 자바에서 자주 발생하는 nullPointer 예외를 감싼/포장 클래스
        ...
        if( optional.isPresent() ){ // .isPresent() : 본문이 존재하는지 검사
            ...
            entity.setCol2(examEntity.getCol2());   // setter 이용한 엔티티값 수정
            entity.setCol3(examEntity.getCol3());   // setter 이용한 엔티티값 수정 ,update 자동 처리
            return entity;
        }
        return examEntity;
    }

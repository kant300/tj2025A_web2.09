...
public class ChatHandler extends TextWebSocketHandler {
    ...
    // *********** 서버에 접속된 클라이언트 소켓들에게 명단 목록  **************
    private static final List< WebSocketSession > 접속명단 = new Vector<>();
        // * ArrayList 동기화 안됨, Vector 동기화 지원 : 채팅은 동시다발적으로 요청이 있으므로 동기화
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        // 메시지를 받을 세션.sendMessage()
        // 서버로부터 메시지를 보내온 클라이언트 소켓에게 메시지는 보내는 예제

        // 1. 클라이언트 소켓이 보내온 메시지를 **현재 접속된** 다른 클라이언트소켓들에게 메시지를 보내기
        //
        //

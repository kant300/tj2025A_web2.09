...
set SQL_SAFE_UPDATES = 0; -- mysql 웨크벤치에서 update 사용 설정
...
# 1. 트랜잭션 시장
start transaction;
# 2. 여러 작업 (DML:INSERT/UPDATE/DELETE ) , DDL불가능( ALTER,CREATE,DROP )
...
commit;
# 6. 특정한 지점으로 롤백
ROLLBACK TO pointA; -- 저장지점까지 으로 롤백(이동)
# 7. 확인 : (1) 7-1-2-3-4-5-7 : 2/4 적용  (2) 7-1-2-3-4-6(특정롤백)-5-7 : 2번만 적용
select * from trans;

# DDL DML DCL(권한부여) TCL(COMMIT,ROLLBACK,SAVEPOINT)
# 1. JAVA SPRING(@Transcational 사용하되 RuntimeException으로 롤백한다. savepoint지원하지 않는다)
# 2. JAVA JDBC(DAO) 모두 지원 : String sql = "savepoint";

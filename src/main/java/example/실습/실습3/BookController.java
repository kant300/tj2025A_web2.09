...
@Log4j2 // =======로그처리하는 어노테이션 제공==========
public class BookController {
    private final BookService bookService;
    // 2. print 함수 대신에 로그함수 사용해보기 = 부가기능 제공, 제약조건등
    @GetMapping("/log") public void log(){
        System.out.println("개발단계에서는 print 정말 많이 작성하자...");
        // log.XXXX : 출력함수처럼 출력하는 메시지 함수이면서 부가기능(파일처리/제약조건 등)
        log.debug("테스트 과정 사용된다.");   // 테스트(개발) 과정에서 메시지
        log.info("서비스의 흐름, 데이터 상태 사용된다.");  // 운용과정 메시지
        log.warn("잠재적 문제 사용된다.");   // 유지보수 과정 메시지
        log.error("예외/실패/오류 상황 사용된다."); // 운용, 유지보수 과정 메시지
    }
    ...
    public ResponseEntity<Boolean> rent(
        ...
        }catch (RuntimeException e){ // 만약에 롤백이면 , e.getMessage() : 예외멧지

...
public class ResponseController {
    ...
    public ResponseEntity< Boolean > task1(){
        return ResponseEntity.status( 401 ).body( false ); // 401 : 인증실패 뜻
    }

    // 2.
    @GetMapping("/int")
    public ResponseEntity< Integer > task2(){
        return ResponseEntity.status( 202 ).body( 1); //  202 : 요청성공했지만 처리중
    }
    ...
    public ResponseEntity< Void > task4(){
        return ResponseEntity.status( 403 ).build(); // 403 : 접근 권한없다 . build() : 반환값없다.
    }

...
const obj = { name : "유재석" }; // 변수,상수에는 객체/함수/배열 담기 가능
const method = () => {} // 변수,상수에는 함수를 담기 기능
const arr = [ "유재석" , "강호동" ]

// * var 키워드 : let 키워드 없었던 시절에 사용된 키워드
var count3 = 30;
var count3 = 40;    // var 키워드는 중복 변수명을 허용한다. 식별이 어렵다
// -> let 키워드는 중복변수명을 허용하지 않는다.
...
// [3-2] 조건문2 : 삼항연산자 , 조건 ? 참 : 거짓, 간단한 조건에서 주로 사용됨
console.log( point >= 90 ? "A학점" : point >=80 ? "B학점" : "C학점");

// [3-2] 조건문3 : 단축평가 , 조건 && 참이면 결과, 조건 || 거짓이면 결과
...
// ** map ** : forEach와 다르게 return 가능하다. ** 
let newArray = array.map( (value) => { console.log( value ); } )
// ** filter ** : 조건부 return 가능하다.
let newArray2 = array.filter( (value) => { console.log( value ); return value > 20; } )

// [5] 함수 :
function fun1( param1, param2 ){} // 5-1 선언적 함수
...
// [6] 객체 : 여러개의 값을 가진 (하나의)값, (1개)가방안에 여러개의 값을 포함 
// 변수/상수는 값을 저장하는 상징적인 이름
const obj1 = { name : "유재석" , age : 40 }
const name2 = "강호동";
const age2 = 50;
const obj2 = { name2 , age2 }; // key와 value의 변수명이 같으면 key 생략가능 
const obj3 = [ "유재석" , 40 ]
console.log( obj3[0] );
// *** 스프레드 연산자 : ...배열이나 객체를 복사할때 사용, 왜? 주소값 변경 목적
const obj4 = { ...obj1 , phone : "010" }; console.log(obj4);
const obj5 = [ ...obj3 ]; console.log( obj5 );  // 값의 차이는 없지만 새로운 주소값으로 복사
const obj6 = [ 6, 7 ,...obj3 ];  console.log( obj6 );
obj6.push( obj6 ); console.log( obj6 );
...
// [9] async/await 동기화
    // 1. 비동기, fetch , fetch함수는 원래 비동기 래퍼( new Promise)이다.
const method1 = () => {
    fetch( "url" )

...
client.onmessage = ( event ) => {
    ...
    }else if( message.type == 'msg' ){ // 메시지 타입이 'msg'이면
        // *** 내가 보낸 메시지 ***
        if( message.from == nickName ){ // 보낸사람이 'nickName'과 같으면
    ...
    // DOM 객체.scrollHeight : 현재 dom(마크업)으ㅢ 스크롤 전체 길이
}
...
const onMsgSend = () => {
    ...
    if( message == '' ) return // 2. 만약에 입력값이 없으면 종료
    // 3. 메시지 구성하기
    const msg = { 
        type : 'msg' , message : message , // type : 메시지종류(msg/join/alarm) , message:메시지내용물
        from : nickName , date : new Date().toLocaleDateString() //from :보낸사람, date : 현재날짜/산
    } 
...
const enterKey = () => {
    // 만약에 input 에서 enter(13)키를 눌렀을때 , keyCode에서는 enter가 13이다.

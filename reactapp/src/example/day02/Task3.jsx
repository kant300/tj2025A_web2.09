...
//     }
//     return(<>
//         <h5> 제품명 : <input value={data } onChange={dataAdd} /> </h5>
//         <h5> 현재 수량 : { count } </h5>
//         <button onClick={ countAdd } > 증가 </button><button onClick={ disCount }> 감소 </button>
//     </>)
// }
...
export default function Task3( props ){
    ...
    const [ count, setCount ] = useState( 0 ); // useState(초기값);
        // 1-2. 증가함수, setXXX( 수정할값 ): 수정할 값을 대입하여 새로고침(재렌더링/함수재실행) 한다.
    const countIn =()=>{ setCount( count +1 ); }
        // 1-3. 감소함수
    const countDe =()=>{ setCount( count -1 ); }

    // [2] 현재 입력받은 제품명 관리하는 useState
        // 2-1 선언

...
export default function Component7 ( props ){
    // [1] useState 변수 선언 : 구문분해 이용한 useState 반환값을 변수화
    // 1. import{ useState } from "react"
    // 2. const [변수명, set변수명] = useState(초기값);
    //  * 변수명은 카멜표기법, set변수명은 변수명앞에 set 붙인다.
    const [ count , setCount ] = useState( 0 );
    // 3. 특정한 useState 값을 변경,
    const countAdd =()=>{  //  () : 클릭이벤트결과정보  
        // setXXX(새로운값); // 만일 값이 변경되었을때 해당 컴포넌트 재실행(재렌더링)
        // 주의할점 : (주소)값이 변경되는 전제조건,  1 -> 2 , 유재석 -> 강호동 , [ 1 , 2 ] -> [ 1,2,3 ]
        const newValue = count +1;
        setCount( newValue ); // 훅(갈고리:특정한 기능을 실행하면 다른 기능들도 실행)
    }

    // [2] 
    const [ array, setArray ] = useState( ['수박'] ); // 빈배열만들어서
    const arrayAdd =( 클릭이벤트결과정보 )=>{ 
        // 주의할점은 useState는 수박을 관리하는게 아니라 수박을 포함하는 []를 관리함.
        array.push('사과'); // [빈배열에] ['사과'] 추가 : 데이터 2개 , ['수박'] --> [ '수박', '사과' ] 
    ...
    const dataAdd = ( event )=> { // (event : 이벤트결과정보)
        // onChange가 실행되었을때 event(이벤트결과정보)가 함수의 매개변수로 전달된다.
        console.log( event );
        console.log( event.target ); // onChange가 발동한 마크업가져오기
...
/* 
    1. 데이터/자료 : 값
    2. 자료타입 : 값의 분류
        기본타입(리터럴:미리 만들어진 데이터들) VS 참조타입(객체/주소값)
    3. 자료의 주소값 변경 기준
        1 -> 2 : 리터럴/(주소) 변경
        'a' -> 'b' : 주소변경

...
export default function Component13( props ){
    // [1] store 저장된 상태를 가져오기 , useSelector( (state) => state.store등록된상태명 )
    // const { 상태변수명 } = useSelector( (state) => state.슬라이스명 )
    const { isAuthenticated } = useSelector( (state) => state.user );
    console.log( isAuthenticated )

    // [2] 액션 이용한 전역변후 상태 변경한다. useDispatch(); 
    const dispatch = useDispatch();
    // [4] 로그인 처리
    const loginHandle =()=>{ // 로그인 axios 했다 가정하에
        dispatch( login() ); // dispatch 이용한 login 액션함수 실행. dispatch( login리듀서 )
    }
    // [5] 로그아웃 처리
    const logoutHandle =()=>{ // 로그아웃 axios 했다 가정하에
        dispatch( logout() ); // dispatch 이용한 logout 액션함수 실행. dispath( logout리듀서 )
    }
    return(<>

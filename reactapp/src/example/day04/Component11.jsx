...
export default function Component11(props){
    ...
    // [1] 렌더링하지 않고 데이터를 참조하는 훅 , useRef vs useState
    const inputRef = useRef ( null ) // 1. import { useRef } from "recat" 또는 자동완성
                                     // 2. useRef( 초기값 )
    const  등록함수 = () => {
        ...
        console.log(inputRef.current.value);    // 단순입력이라면 useState 보다 useRef 코드가 단순하다
   ...
   const countRef = useRef( count ); // 1.useRef(초기값)
    ...
    } , [ count ] );    // count의 상태가 변경되면 해당함수(자동) 실행

    // [3] 
    const formRef = useRef( );
    const 전송함수 =()=>{
        console.log( formRef.current ); // 폼 내용물들을 한번에 가져와서 한번에 자바(스프링)에게 전송
        console.log( formRef.current.elements['textData'].value); // formRef.current.elements[ name속성명
